...
int main() {
    ...
    vector<string> cards = {
        ...
        "King of Diamonds", "Joker", "Joker"
    };
    
    // Pecatigi kartite po red
    cout << "Cards in order:" << endl;
    for (string card : cards) {
        cout << card << endl;
    }
    
    // Programata ja koristi funkcijata shuffle za slucajno mesanje na kartite
    random_device rd;
    mt19937 g(rd());
    shuffle(cards.begin(), cards.end(), g);
    
    // Programata gi pecati izmesanite karti so jamka
    cout << "\nShuffled cards:" << endl;
    for (string card : cards) {
        cout << card << endl;
    }
